...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<Either<Failure, UserEntity>> login(String email, String password, String role) async {
    try {
      final loginResponseDto = await remoteDataSource.login(email, password, role);
  ...
  @override
  Future<Either<Failure, UserEntity>> register({required String fullName, required String email, required String phone, required String password, required String role}) async {
    try {
      final userDto = await remoteDataSource.register(fullName: fullName, email: email, phone: phone, password: password, role: role);
  ...
  Future<Either<Failure, UserEntity?>> checkAuthStatus() async {
    ...
    } catch (e) { return Left(CacheFailure(message: 'Local storage error.')); }
  }
  ...
  Future<Either<Failure, void>> logout() async {
    try {
      ...
      await localDataSource.clearCache();
      return const Right(null);
    } catch (e) { return Left(CacheFailure(message: 'Failed to clear local data.')); }
  }

  @override
  Future<Either<Failure, UserEntity>> updateProfile({String? fullName, File? avatar, File? resume}) async {
    try {
      final userDto = await remoteDataSource.updateProfile(fullName: fullName, avatar: avatar, resume: resume);
      await localDataSource.cacheUser(userDto.toHiveModel());
      return Right(userDto.toEntity());
    } on ServerException catch (e) { return Left(ServerFailure(message: e.message)); }
  }
